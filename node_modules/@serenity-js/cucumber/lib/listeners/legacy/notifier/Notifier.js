...
class Notifier {
    ...
    stepStarts(step) {
        ...
        this.emit(new events_1.TaskStarts(this.currentSceneId, this.currentStepActivityId, new model_1.ActivityDetails(step.name), this.serenity.currentTime()));
    }
    stepFinished(step, outcome) {
        this.emit(new events_1.TaskFinished(this.currentSceneId, this.currentStepActivityId, new model_1.ActivityDetails(step.name), outcome, this.serenity.currentTime()));
    }
    scenarioFinishes(scenario, feature, outcome) {
        this.emitSceneFinishes(this.detailsOf(scenario, feature), outcome);
    }
    currentScenarioFinishes(outcome) {
    ...
    emitSceneFinishes(details, outcome) {
        this.emit(new events_1.SceneFinishes(this.currentSceneId, details, outcome, this.serenity.currentTime()));
    }
